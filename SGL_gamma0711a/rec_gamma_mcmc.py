...
#np.save(os.path.join(self.output_folder,f'all_ln_probs{self.mode}.npy'),all_ln_probs)
    ...
    def calculate_statistics(self):
        ...
        for sample in self.all_samples:
            ...
            #print(np.shape(self.all_samples), np.shape(sample), np.shape(sample[:, 0]))
        ...
        if self.evaluation:
            if self.ndim==1:
                ...
                for  i in range(len(self.lens_table)):
                    ...
                    peak_p = marginal_peak_and_mad(samples1)[0]
                    max_p = lnprob(peak_p, theta_E_r_list[i], theta_ap_r_list[i], sigma_ap_list[i], dd_list[i], abs_delta_sigma_ap_list[i], abs_delta_dd_list[i])
                    aic_tmp =  AIC(self.ndim, max_p)
                    bic_tmp = BIC(self.ndim,len(self.lens_table),max_p)
                ...
                #for row in self.lens_table:
                #    zl_list, theta_E_r_list, theta_ap_r_list, sigma_ap_list, dd_list, abs_delta_sigma_ap_list, abs_delta_dd_list = self.get_parameters(row,zl_list, theta_E_r_list, theta_ap_r_list, sigma_ap_list, dd_list, abs_delta_sigma_ap_list, abs_delta_dd_list)
                samples1 = self.all_samples
                peak_p = marginal_peak_and_mad(samples1)[0]
                max_p = self.lnprob_touse(peak_p,zl_list, theta_E_r_list, theta_ap_r_list, sigma_ap_list, dd_list, abs_delta_sigma_ap_list, abs_delta_dd_list)

                aic = AIC(self.ndim, max_p)
                bic = BIC(len(self.lens_table), self.ndim, max_p)
