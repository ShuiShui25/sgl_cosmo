...
def chi_2(x, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    ...
    denom_th = (4 * (abs_delta_sigma_ap / sigma)**2 + ((1 - x) * 0.05)**2 )* d_th(x, theta, theta_ap, sigma)**2.0
...
def chi_2_K(x,theta, theta_ap, sigma,dd,abs_delta_sigma_ap,abs_delta_dd,delta, beta):
    ...
    denom_th = (4*(abs_delta_sigma_ap/sigma)**2 + ((1-x)*0.05)**2) * d_th_prime(x,theta, theta_ap, sigma,delta, beta)**2.0
    denom_obs =  (abs_delta_dd)**2
...
def lnprob_K_3D_fixdelta_Guerrini2024(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, beta = g
    total_lnlike = 0
    d0 = 2.31
    d1 = 0.0
    ...
    mu, sigma_beta = 0.22, 0.1  # Adjust these values as needed
    ...
    for i in range(len(theta)):
        ...
        if not (1.2 < x < 2.8 and 1.7 < delta < 2.8 and f_g > 0.):
            return -np.inf
...
def lnprob_K_3D(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    ...
    a, c, b = -0.5, 0.102, 0.656  # Adjust these values as needed
    ...
    for i in range(len(theta)):
        ...
        # Check if x, delta, and f_g are within the desired range
        if not (1.2 < x < 2.8 and 1.5 < delta < 2.8 and f_g > 0.):
            return -np.inf
...
def lnprob_K_3D_sim(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, d0, beta = g
    total_lnlike = 0
...
### ===========================================================================
### Gaussian Guerrini2024

def lnprob_K_3D_Guerrini2024(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, d0, beta = g
    total_lnlike = 0
...
def lnprob_K_3D_Bolton2006(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
  g0, d0, beta = g
  total_lnlike = 0

  # Define the mean (mu) and standard deviation (sigma) of the Gaussian distribution for beta
  mu, sigma_beta = 0.18, 0.13  # Adjust these values as needed
...
# ##============================================================================
# ## Gaussian prior Guerrini2024

def lnprob_K_5D_Guerrini2024(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
...
# ##============================================================================
# ## Gaussian prior Bolton2006

def lnprob_K_5D_Bolton2006(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
...
# ##============================================================================
# ### CPL, Gaussian prior Guerrini2024

def lnprob_K_5D_scale_Guerrini2024(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    ## scale evolved
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
...
# ###============================================================================
# ### CPL, Gaussian prior Bolton2006

def lnprob_K_5D_scale_Bolton2006(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    ## scale evolved
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
...
# ##============================================================================
# ## Gaussian prior Tri linear sigma evo

def lnprob_K_5D_sigma_Tri(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
    ...
    # Iterate over each row in the fits table
    for i in range(len(theta)):
        x = g0+g1*(sigma[i]/np.mean(sigma))
        delta = d0+d1*(sigma[i]/np.mean(sigma))
        f_g = f_prime(x, delta, beta)
...
# ##============================================================================
# ## Gaussian prior Guerrini2024 linear sigma evo

def lnprob_K_5D_sigma_Guerrini2024(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
...
# ##============================================================================
# ## Gaussian prior Bolton2006 linear sigma evo

def lnprob_K_5D_sigma_Bolton2006(g, zl, theta, theta_ap, sigma, dd, abs_delta_sigma_ap, abs_delta_dd):
    g0, g1, d0, d1, beta = g
    total_lnlike = 0
