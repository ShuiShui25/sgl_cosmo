...
from mcmc_utils import lnprob_K_5D_Guerrini2024,lnprob_K_5D_Bolton2006
from mcmc_utils import lnprob_K_5D_scale,lnprob_K_5D_scale_Guerrini2024,lnprob_K_5D_scale_Bolton2006
from mcmc_utils import lnprob_K_5D_sigma_Guerrini2024,lnprob_K_5D_sigma_Tri,lnprob_K_5D_sigma_Bolton2006
import numpy as np
...
lens_table_path = os.path.join(path_project, 'Data' , 'SGLTable04.fits')
nwalkers = 200
nsteps = 20000
...
#this can be None
name_model_out_list = ['ANNgood-sigma-Tri']
...
for name_model, table,model_name_out  in zip(name_model_list, table_list, name_model_out_list) :
    ...
    # print(f" \n ************** gamma from {table} for fixed a bins ************** \n")
    # zl_min = 0.0625
    # zl_max = 1.004
    # a_max = 1./(1.+zl_min)
    # a_min = 1./(1.+zl_max)
    # abin_width = np.linspace(a_min,a_max,6)[1]-np.linspace(a_min,a_max,6)[0]
    # ## run the mcmc for fixed bins
    # mcmc_fixed_abins = MCMC(lens_table_path = table , model=name_model, bin_width=abin_width,
    #                         mode='1D',  type_step = 'a',
    #                         path_project = path_project, nwalkers = nwalkers, nsteps = nsteps,
    #                         checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
    #                         model_name_out =model_name_out)
    # mcmc_fixed_abins.main()
    # print('Done! \n')
    
    # if mcmc_linear:
    #     print(mcmc_fixed_abins.path_output_table)
    #     run_linearfit(mcmc_fixed_abins.path_output_table, mcmc_fixed_abins.output_folder)
    
    # print(f" \n ************** gamma from {table} for fixed z bins ************** \n")
    # ## run the mcmc for fixed bins
    ...
    # mcmc_K = MCMC(lens_table_path = table,
    #                 path_project=path_project,
    #             model=name_model, nwalkers=400, nsteps = 20000, mode='Koopmans_5D',  x_ini= [2.0, 0, 2.0, 0, 0],
    #             checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
    #             model_name_out =model_name_out,  
    #             lnprob_touse= lnprob_K_5D_Guerrini2024)
    ...
    #             checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
    #             model_name_out =model_name_out,  
    #             lnprob_touse= lnprob_K_5D_scale_Guerrini2024)
    ...
    #             checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
    #             model_name_out =model_name_out,  
    #             lnprob_touse= lnprob_K_5D_Bolton2006,
    #             evaluation= True)
    ...
    #             checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
    #             model_name_out =model_name_out,  
    #             lnprob_touse= lnprob_K_5D_scale_Bolton2006,
    #             evaluation= True)
...
#                checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
#                model_name_out =model_name_out,  
#                lnprob_touse= lnprob_K_5D_sigma_Guerrini2024)
#    
#    mcmc_K.main()
#    print('Done! \n')
    ...
    mcmc_K = MCMC(lens_table_path = table,
                ...
                checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
                model_name_out =model_name_out,  
                lnprob_touse= lnprob_K_5D_sigma_Tri)
...
#                checkpoint=checkpoint, ncpu=ncpu, color_points=color_points,
#                model_name_out =model_name_out,  
#                lnprob_touse= lnprob_K_5D_sigma_Bolton2006)
#    
#    mcmc_K.main()
#    print('Done! \n')
