#!/usr/bin/env python3
"""ANN ensemble reconstruction with inverse-density weighting applied in residual target construction."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_inverse_density_rtarget_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble on a supplied redshift frame, applying inverse-density weighting "
            "inside the residual r(z) target construction."
        ),
        epilog=(
...
def build_training_targets(
    ...
    density_weight_strength: float,
    density_weight_power: float,
    density_k: int,
    min_density_spacing: float,
) -> np.ndarray:
    order = np.argsort(z_obs)
    z_sorted = z_obs[order]
    m_sorted = m_values[order]

    residual_sorted = m_sorted - baseline_magnitude_shape(z_sorted)
    z_unique, inverse, counts = np.unique(z_sorted, return_inverse=True, return_counts=True)
    residual_unique = np.bincount(inverse, weights=residual_sorted) / counts

    density_weight_unique = make_inverse_density_sample_weight(
        z_values=z_unique,
        strength=density_weight_strength,
        power=density_weight_power,
        k=density_k,
        min_spacing=min_density_spacing,
    )
    density_weight_unique = np.asarray(density_weight_unique, dtype=float)
    density_weight_unique = np.clip(density_weight_unique, 1.0e-6, None)
    density_weight_unique /= np.median(density_weight_unique)
    density_weight_unique = np.clip(density_weight_unique, 0.25, 25.0)

    if z_unique.size < 4:
        smoothed_unique = residual_unique.copy()
        derivative_unique = np.gradient(smoothed_unique, z_unique, edge_order=1)
    else:
        smoothing = max(float(derivative_smoothing * z_unique.size), 1.0e-8)
        spline_success = False
        for smoothing_factor in (1.0, 5.0, 20.0, 100.0):
            try:
                with warnings.catch_warnings(record=True) as caught:
                    ...
                    spline = UnivariateSpline(
                        z_unique,
                        residual_unique,
                        w=density_weight_unique,
                        s=smoothing * smoothing_factor,
                        k=min(3, z_unique.size - 1),
                    )
                    smoothed_try = spline(z_unique)
                    derivative_try = spline.derivative()(z_unique)

                if any("maximal number of iterations maxit" in str(w.message) for w in caught):
                    continue
                if not (np.all(np.isfinite(smoothed_try)) and np.all(np.isfinite(derivative_try))):
                    continue

                smoothed_unique = smoothed_try
                derivative_unique = derivative_try
                spline_success = True
                break
            except Exception:
                continue
        ...
        if not spline_success:
            bandwidth = max(float(derivative_smoothing), float(min_density_spacing), 5.0e-3)
            smoothed_unique = np.empty_like(residual_unique)
            for idx, z0 in enumerate(z_unique):
                scaled = (z_unique - z0) / bandwidth
                kernel = np.exp(-0.5 * scaled**2) * density_weight_unique
                kernel_sum = np.sum(kernel)
                if kernel_sum <= 0.0 or not np.isfinite(kernel_sum):
                    smoothed_unique[idx] = residual_unique[idx]
                else:
                    smoothed_unique[idx] = np.sum(kernel * residual_unique) / kernel_sum
            derivative_unique = np.gradient(smoothed_unique, z_unique, edge_order=1)

    smoothed_unique = np.nan_to_num(smoothed_unique, nan=0.0, posinf=0.0, neginf=0.0)
    derivative_unique = np.nan_to_num(derivative_unique, nan=0.0, posinf=0.0, neginf=0.0)
    derivative_unique = np.clip(derivative_unique, -1.0e4, 1.0e4)

    residual_sorted_target = smoothed_unique[inverse]
    derivative_sorted_target = derivative_unique[inverse]
    residual_target = np.empty_like(residual_sorted_target)
    derivative_target = np.empty_like(derivative_sorted_target)
    residual_target[order] = residual_sorted_target
    derivative_target[order] = derivative_sorted_target
    return np.column_stack([residual_target, derivative_target])
...
def estimate_loss_scales(z_obs: np.ndarray, train_targets: np.ndarray) -> dict[str, float]:
    ...
    if not np.all(np.isfinite(train_targets)):
        raise ValueError("Training targets contain non-finite values.")

    mb_scale = float(np.nanstd(train_targets[:, 0], ddof=1))
    dmb_scale = float(np.nanstd(train_targets[:, 1], ddof=1))
    physical_m = train_targets[:, 0] + baseline_magnitude_shape(z_obs)
    shape_values = distance_shape_proxy(z_obs, physical_m)
    shape_scale = float(np.nanstd(shape_values, ddof=1))
    return {
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        ...
        total_loss = mb_mse + args.derivative_weight * dmb_mse
        return -total_loss
...
def main() -> None:
    ...
    print("Best CV score (neg target-constructed residual loss):", best_cv_score)
        ...
        json.dumps(
            {
                ...
                "target_transform": "inverse-density-weighted residual target construction relative to 5*log10(z)",
                "loss_scales": loss_scales,
                "density_weight_strength": args.density_weight_strength,
                "density_weight_power": args.density_weight_power,
                "density_k": args.density_k,
                ...
                "distance_curvature_penalty_weight": 0.0,
                "derivative_smoothing": args.derivative_smoothing,
