#!/usr/bin/env python3
"""ANN ensemble reconstruction of Pantheon+ corrected apparent magnitude with adaptive local length scales."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_variable_lengthscale_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble on a supplied redshift frame, using adaptive local length scales "
            "to smooth sparse-redshift regions more strongly."
        ),
        epilog=(
    ...
    parser.add_argument(
        "--highz-weight-strength",
        type=float,
        default=1.0,
        help="Additional sample-weight strength toward the high-redshift end of the training sample.",
    )
    parser.add_argument(
        "--highz-weight-power",
        type=float,
        default=1.5,
        help="Power-law index controlling how quickly the training weight rises with redshift.",
    )
    parser.add_argument(
        "--length-scale-k",
        type=int,
        default=25,
        help="Neighbor rank used to estimate the local redshift spacing for adaptive smoothing.",
    )
    parser.add_argument(
        "--length-scale-factor",
        type=float,
        default=3.0,
        help="Multiplicative factor applied to the local spacing to set the adaptive bandwidth.",
    )
    parser.add_argument(
        "--min-bandwidth",
        type=float,
        default=0.015,
        help="Lower floor on the adaptive local bandwidth in redshift.",
    )
...
def estimate_derivative_targets(
    ...
    if z_unique.size < 4:
        ...
        derivative_unique = np.empty_like(m_unique)
        k_index = min(max(int(length_scale_k), 1), z_unique.size - 1)
        for idx, z0 in enumerate(z_unique):
            distances = np.abs(z_unique - z0)
            local_spacing = np.partition(distances, k_index)[k_index]
            bandwidth = max(
                float(min_bandwidth),
                float(smoothing_strength) * float(length_scale_factor) * float(local_spacing),
            )
            u = (z_unique - z0) / bandwidth
            weights = np.exp(-0.5 * u**2)
            design = np.column_stack(
                [
                    np.ones_like(z_unique),
                    z_unique - z0,
                    (z_unique - z0) ** 2,
                ]
            )
...
def cross_validate_hyperparameters(
    ...
    sample_weight = make_highz_sample_weight(
        z_values=z_obs,
        strength=args.highz_weight_strength,
        power=args.highz_weight_power,
    )
...
def fit_single_realization(
    ...
    train_targets = build_training_targets(
        ...
        length_scale_k=args.length_scale_k,
        ...
        json.dumps(
            {
                ...
                "loss_scales": loss_scales,
                "highz_weight_strength": args.highz_weight_strength,
                "highz_weight_power": args.highz_weight_power,
                "length_scale_k": args.length_scale_k,
                "length_scale_factor": args.length_scale_factor,
                "min_bandwidth": args.min_bandwidth,
