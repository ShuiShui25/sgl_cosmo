#!/usr/bin/env python3
"""ANN ensemble reconstruction of Pantheon+ corrected apparent magnitude with high-z-emphasized CV scoring."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_highz_score_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble on a supplied redshift frame, with CV scoring that emphasizes high redshift."
        ),
        epilog=(
    ...
    parser.add_argument(
        "--highz-score-strength",
        type=float,
        default=1.0,
        help="Additional CV-score weight strength toward the high-redshift end of each validation fold.",
    )
    parser.add_argument(
        "--highz-score-power",
        type=float,
        default=1.5,
        help="Power-law index controlling how quickly the CV-score weight rises with redshift.",
    )
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        ...
        score_weight = make_highz_score_weight(
            z_values=x_val[:, 0],
            strength=args.highz_score_strength,
            power=args.highz_score_power,
        )
        mb_resid = (pred[:, 0] - y_val[:, 0]) / loss_scales["mb_scale"]
        dmb_resid = (pred[:, 1] - y_val[:, 1]) / loss_scales["dmb_scale"]
        mb_mse = float(np.average(mb_resid**2, weights=score_weight))
        dmb_mse = float(np.average(dmb_resid**2, weights=score_weight))
        ...
        json.dumps(
            {
                ...
                "loss_scales": loss_scales,
                "highz_score_strength": args.highz_score_strength,
                "highz_score_power": args.highz_score_power,
