#!/usr/bin/env python3
"""ANN ensemble reconstruction with inverse-density weighting applied to the residual r(z) CV error term."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_inverse_density_rerror_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "to the residual r(z) cross-validation error term."
        ),
        epilog=(
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        ...
        residual_weight = make_inverse_density_sample_weight(
            z_values=x_val[:, 0],
        ...
        mb_mse = float(np.average(mb_resid**2, weights=residual_weight))
...
def main() -> None:
    ...
    print("Best CV score (neg density-weighted residual loss):", best_cv_score)
