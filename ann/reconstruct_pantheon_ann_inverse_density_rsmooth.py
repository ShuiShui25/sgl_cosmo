#!/usr/bin/env python3
"""ANN ensemble reconstruction with inverse-density-weighted residual smoothness regularization."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_inverse_density_rsmooth_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble on a supplied redshift frame, using inverse-density weighting "
            "to reduce residual r(z) smoothness regularization in sparse-redshift regions."
        ),
        epilog=(
    ...
    parser.add_argument(
        "--residual-smoothness-weight",
        type=float,
        default=0.05,
        help="Weight of the inverse-density-weighted smoothness penalty on residual r(z).",
    )
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        ...
        z_sorted = np.asarray(x_val[order, 0], dtype=float)
        residual_sorted = np.asarray(pred[order, 0] / loss_scales["mb_scale"], dtype=float)
        ...
        if z_unique.size < 5:
            ...
            first = np.gradient(residual_unique, z_unique, edge_order=1)
            second = np.gradient(first, z_unique, edge_order=1)
            density_weight = make_inverse_density_sample_weight(
                z_values=z_unique,
                strength=args.density_weight_strength,
                power=args.density_weight_power,
                k=args.density_k,
                min_spacing=args.min_density_spacing,
            )
            density_weight = np.clip(np.asarray(density_weight, dtype=float), 1.0e-8, None)
            second = np.nan_to_num(second, nan=0.0, posinf=0.0, neginf=0.0)
            smooth_penalty = float(np.mean((second**2) / density_weight))

        total_loss = mb_mse + args.derivative_weight * dmb_mse + args.residual_smoothness_weight * smooth_penalty
        return -total_loss
...
def cross_validate_hyperparameters(
    ...
    sample_weight = make_inverse_density_sample_weight(
        z_values=z_obs,
        strength=args.density_weight_strength,
        power=args.density_weight_power,
        k=args.density_k,
...
def main() -> None:
    ...
    print("Best CV score (neg residual+derivative+sparsity-aware smoothness loss):", best_cv_score)
