#!/usr/bin/env python3
"""ANN ensemble reconstruction with unweighted training that learns only residual r(z)."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_unweighted_ronly_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and learning only the residual r(z)."
        ),
        epilog=(
...
def estimate_loss_scales(z_obs: np.ndarray, train_targets: np.ndarray) -> dict[str, float]:
    ...
    mb_scale = float(np.std(train_targets, ddof=1))
    dmb_scale = 1.0
    physical_m = train_targets + baseline_magnitude_shape(z_obs)
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        pred = np.asarray(estimator.predict(x_val), dtype=float).reshape(-1)
        y_val = np.asarray(y_val, dtype=float).reshape(-1)
        mb_resid = (pred - y_val) / loss_scales["mb_scale"]
        mb_mse = float(np.mean(mb_resid**2))

        order = np.argsort(x_val[:, 0])
        z_sorted = x_val[order, 0]
        physical_m_sorted = pred[order] + baseline_magnitude_shape(z_sorted)
        ...
        total_loss = mb_mse + args.distance_curvature_penalty_weight * distance_penalty
        return -total_loss
...
def fit_single_realization(
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        with threadpool_limits(limits=args.blas_threads):
            ...
            prediction_residual = np.asarray(model.predict(x_frame), dtype=float).reshape(-1)

    unconverged = any(issubclass(w.category, ConvergenceWarning) for w in caught)
    mb_pred = prediction_residual + baseline_magnitude_shape(z_frame)
    dmb_pred = np.gradient(mb_pred, z_frame, edge_order=1)
...
def main() -> None:
    ...
    print("Best CV score (neg residual-only loss):", best_cv_score)
        ...
        json.dumps(
            {
                ...
                "variant": "unweighted_ronly",
                "target_transform": "residual relative to 5*log10(z) only",
                "loss_scales": loss_scales,
