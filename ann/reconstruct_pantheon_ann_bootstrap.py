#!/usr/bin/env python3
"""Bootstrap ANN ensemble reconstruction of Pantheon+ corrected apparent magnitude on a target redshift frame."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir / "output" / "mb_ann_bootstrap_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble using true bootstrap resampling on a supplied redshift frame."
        ),
        epilog=(
            "Parallel control: --n-jobs sets how many bootstrap realizations or CV fits are run in "
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Number of bootstrap resampled training sets.",
    )
    parser.add_argument(
        "--cv-folds",
        type=int,
...
def bootstrap_indices(n_obs: int, n_realizations: int, rng: np.random.Generator) -> np.ndarray:
    return rng.integers(0, n_obs, size=(n_realizations, n_obs), endpoint=False)


def fit_single_bootstrap_realization(
    idx: int,
    ...
    z_boot = z_obs[bootstrap_idx]
    m_boot = m_obs[bootstrap_idx]
    x_train = z_boot.reshape(-1, 1)
...
def reconstruct_mb_grid_bootstrap(
    z_obs: np.ndarray,
    m_obs: np.ndarray,
    z_frame: np.ndarray,
    ...
    mb_predictions = np.empty((bootstrap_ids.shape[0], z_frame.size), dtype=float)
    dmb_predictions = np.empty((bootstrap_ids.shape[0], z_frame.size), dtype=float)
    unconverged = np.zeros(bootstrap_ids.shape[0], dtype=bool)
    seeds = rng.integers(0, 2**31 - 1, size=bootstrap_ids.shape[0], dtype=np.int64)

    tasks = (
        delayed(fit_single_bootstrap_realization)(
            idx=idx,
            bootstrap_idx=bootstrap_ids[idx],
            z_obs=z_obs,
            m_obs=m_obs,
            z_frame=z_frame,
            args=args,
        ...
        for idx in range(bootstrap_ids.shape[0])
    )

    progress_cls = tqdm if tqdm is not None else SimpleProgress
    with progress_cls(total=bootstrap_ids.shape[0], desc="Training bootstrap realizations", unit="net") as pbar:
...
def main() -> None:
    ...
    print("Best CV score (neg composite loss):", best_cv_score)

    print(
        "Generating true bootstrap realizations:",
        f"{args.n_realizations} resampled training sets",
    )
    bootstrap_ids = bootstrap_indices(n_obs=len(z_obs), n_realizations=args.n_realizations, rng=rng)

    print("Training joint bootstrap MLP ensemble for m_B and dm_B/dz and evaluating on the target frame...")
    mb_frame_samples, dmb_frame_samples, unconverged = reconstruct_mb_grid_bootstrap(
        z_obs=z_obs,
        m_obs=m_obs,
        z_frame=z_frame,
    ...
    csv_path = args.output_dir / "pantheon_mb_ann_bootstrap_reconstruction.csv"
    npz_path = args.output_dir / "pantheon_mb_ann_bootstrap_reconstruction.npz"
    json_path = args.output_dir / "pantheon_mb_ann_bootstrap_best_params.json"

    result_table.to_csv(csv_path, index=False)
    np.savez_compressed(
        npz_path,
        realization_method="bootstrap",
        z_frame=z_frame,
        z_obs=z_obs,
        m_b_obs=m_obs,
        m_b_diag_sigma=m_diag_sigma,
        selection_mask=selection_mask,
        bootstrap_indices=bootstrap_ids,
    ...
    print(f"Saved bootstrap reconstructed m_B grid to: {csv_path}")
    print(f"Saved bootstrap covariance products to: {npz_path}")
    print(f"Saved bootstrap best CV settings to: {json_path}")
    print(
        "Convergence summary:",
