#!/usr/bin/env python3
"""ANN ensemble reconstruction with occupancy weighting and redshift-binned cross-validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    output_dir = script_dir.parent / "output" / "mb_ann_unweighted_binnedcv_run"
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scikit-learn MLP ensemble on a supplied redshift frame, using occupancy weighting "
            "and redshift-binned stratified cross-validation."
        ),
        epilog=(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional strong-lensing FITS table. If provided, build the reconstruction frame "
            "from the sorted unique redshifts appearing in zl and zs, then write a new output FITS with "
            "dd_ANN and dd_error_ANN in that FITS file."
        ),
    )
    parser.add_argument("--output-fits", type=Path, default=None,
                        help="New lens FITS path (default: OUTPUT_DIR/SGLTable_ANN_cov.fits).")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=output_dir,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Redshift bin width used to build stratified CV labels from min(zHD).",
    )
    parser.add_argument(
        "--bin-weight-strength",
        type=float,
        default=0.5,
        help="Amplitude of inverse-bin-occupancy weighting; 0 disables weighting and 1 keeps the full contrast.",
    )
    parser.add_argument(
        "--bin-weight-power",
        type=float,
        default=1.0,
        help="Power-law index applied to inverse bin occupancy before amplitude scaling.",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
    ...
    if args.n_realizations < 2 or args.cv_folds < 2 or args.max_iter < 1:
        parser.error("Require n-realizations >= 2, cv-folds >= 2 and max-iter >= 1")
    if args.lens_fits is not None:
        target = args.output_fits or (args.output_dir / "SGLTable_ANN_cov.fits")
        if target.resolve() == args.lens_fits.resolve():
            parser.error("--output-fits must differ from the input lens catalog")
        if target.exists():
            parser.error(f"Output FITS already exists: {target}")
...
def load_lens_frame_from_fits(path: Path) -> tuple[Table, np.ndarray]:
    lens_table = Table.read(path)
    required_columns = {"zl", "zs"}
    missing = required_columns - set(lens_table.colnames)
    if missing:
        raise ValueError(f"Missing required columns in lens FITS {path}: {sorted(missing)}")

    zl = np.asarray(lens_table["zl"], dtype=float).reshape(-1)
    zs = np.asarray(lens_table["zs"], dtype=float).reshape(-1)
    frame = np.unique(np.sort(np.concatenate([zl, zs])))
    return lens_table, frame


def lookup_frame_indices(z_values: np.ndarray, z_frame: np.ndarray) -> np.ndarray:
    z_values = np.asarray(z_values, dtype=float)
    idx = np.searchsorted(z_frame, z_values)
    if np.any(idx >= z_frame.size):
        raise ValueError("Some requested redshifts lie outside the reconstruction frame.")
    if not np.allclose(z_frame[idx], z_values, rtol=0.0, atol=1.0e-12):
        raise ValueError("Some lens redshifts do not exactly match the reconstruction frame.")
    return idx


def compute_dd_from_mb_realizations(
    zl: np.ndarray,
    zs: np.ndarray,
    z_frame: np.ndarray,
    mb_frame_samples: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    idx_l = lookup_frame_indices(zl, z_frame)
    idx_s = lookup_frame_indices(zs, z_frame)

    m_l = mb_frame_samples[:, idx_l]
    m_s = mb_frame_samples[:, idx_s]
    ratio_prefactor = ((1.0 + zs) / (1.0 + zl))[None, :]
    dd_samples = 1.0 - np.power(10.0, (m_l - m_s) / 5.0) * ratio_prefactor
    dd_mean = dd_samples.mean(axis=0)
    dd_sigma = dd_samples.std(axis=0, ddof=1)
    return dd_mean, dd_sigma, dd_samples
...
def make_cv_scorer(args: argparse.Namespace, loss_scales: dict[str, float]):
    def scorer(estimator, x_val, y_val) -> float:
        z_val = np.asarray(x_val[:, 0], dtype=float)
        bin_weights = build_bin_capacity_weights(
            z_obs=z_val,
            bin_width=args.cv_bin_width,
            strength=args.bin_weight_strength,
            power=args.bin_weight_power,
        )
        pred = estimator.predict(x_val)
        mb_resid = (pred[:, 0] - y_val[:, 0]) / loss_scales["mb_scale"]
        dmb_resid = (pred[:, 1] - y_val[:, 1]) / loss_scales["dmb_scale"]
        mb_mse = float(np.average(mb_resid**2, weights=bin_weights))
        dmb_mse = float(np.average(dmb_resid**2, weights=bin_weights))
...
def build_stratified_redshift_labels(
    z_obs: np.ndarray,
    cv_folds: int,
    bin_width: float,
) -> np.ndarray:
    ...
    labels = np.floor((z_obs - z_min) / bin_width).astype(int)

    while True:
        unique_labels, counts = np.unique(labels, return_counts=True)
        sparse = unique_labels[counts < cv_folds]
        if sparse.size == 0:
            break
        for label in sparse:
            if label > labels.min():
                labels[labels == label] = label - 1
            elif label < labels.max():
                labels[labels == label] = label + 1
...
def build_bin_capacity_weights(
    z_obs: np.ndarray,
    bin_width: float,
    strength: float = 0.5,
    power: float = 1.0,
    ...
    bin_counts = np.array([count_map[int(label)] for label in labels], dtype=float)
    reference_count = float(np.mean(counts))
    raw_weights = np.power(reference_count / np.clip(bin_counts, 1.0, None), power)
    normalized_weights = raw_weights / np.mean(raw_weights)
    return 1.0 + strength * (normalized_weights - 1.0)
...
def cross_validate_hyperparameters(
    ...
    strat_labels = build_stratified_redshift_labels(
        z_obs=z_obs,
        cv_folds=args.cv_folds,
        bin_width=args.cv_bin_width,
    )
    sample_weight = build_bin_capacity_weights(
        z_obs=z_obs,
        bin_width=args.cv_bin_width,
        strength=args.bin_weight_strength,
        power=args.bin_weight_power,
    ...
    search = GridSearchCV(
        ...
        cv=cv.split(x_train, strat_labels),
        n_jobs=args.n_jobs,
        refit=True,
    )
    with threadpool_limits(limits=args.blas_threads):
        search.fit(x_train, train_targets, mlp__sample_weight=sample_weight)
...
def fit_single_realization(
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        with threadpool_limits(limits=args.blas_threads):
            ...
            model.fit(x_train, train_targets, mlp__sample_weight=sample_weight)
...
def main() -> None:
    ...
    bin_capacity_weights = build_bin_capacity_weights(
        z_obs=z_obs,
        bin_width=args.cv_bin_width,
    )
    ...
    lens_table = None
    if args.lens_fits is not None:
        lens_table, z_frame = load_lens_frame_from_fits(args.lens_fits)
        frame_label = f"lens FITS frame from {args.lens_fits}"
    ...
    if args.lens_fits is not None:
        print(
            "Lens FITS input:",
            f"{args.lens_fits} with {len(lens_table)} systems",
        )
    print(
        "Parallel settings:",
    ...
    print(
        "Bin-capacity weights:",
        f"bin_width={args.cv_bin_width:.3f}, "
        f"strength={args.bin_weight_strength:.3f}, "
        f"power={args.bin_weight_power:.3f}, "
        f"min={bin_capacity_weights.min():.3f}, "
        f"max={bin_capacity_weights.max():.3f}, "
        f"mean={bin_capacity_weights.mean():.3f}",
    )
    ...
    print("Best CV score (neg composite loss, binned stratified CV):", best_cv_score)
    ...
    if lens_table is not None:
        zl = np.asarray(lens_table["zl"], dtype=float)
        zs = np.asarray(lens_table["zs"], dtype=float)
        dd_mean, dd_sigma, dd_samples = compute_dd_from_mb_realizations(
            zl=zl,
            zs=zs,
            z_frame=z_frame,
            mb_frame_samples=mb_frame_samples,
        )
        lens_table["dd_ANN"] = dd_mean
        lens_table["dd_error_ANN"] = dd_sigma
        updated_lens_fits = args.output_fits or (args.output_dir / "SGLTable_ANN_cov.fits")
        updated_lens_fits.parent.mkdir(parents=True, exist_ok=True)
        lens_table.write(updated_lens_fits, overwrite=False)
    ...
    np.savez_compressed(
        ...
        dd_ann=dd_mean if dd_mean is not None else np.array([]),
        dd_ann_sigma=dd_sigma if dd_sigma is not None else np.array([]),
        dd_ann_realizations=dd_samples if dd_samples is not None else np.empty((0, 0)),
    )
        ...
        json.dumps(
            {
                ...
                "variant": "unweighted_binnedcv",
                "cv_bin_width": args.cv_bin_width,
                "lens_fits": str(args.lens_fits) if args.lens_fits is not None else None,
                "bin_capacity_weighting": {
                    "enabled": True,
                    "scheme": "inverse bin occupancy, normalized to mean 1",
                    "strength": args.bin_weight_strength,
                    "power": args.bin_weight_power,
                },
    ...
    if args.lens_fits is not None:
        updated_lens_fits = args.output_fits or (args.output_dir / "SGLTable_ANN_cov.fits")
        print(f"Saved updated lens FITS with dd_ANN and dd_error_ANN: {updated_lens_fits}")
    print(
        "Convergence summary:",
