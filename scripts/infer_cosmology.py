...
"""
Hierarchical strong-lensing inference, based on the completed sgl146 runs.
See README.md for the distinction between this baseline and published priors.
Features:
...
def load_fits_minimal(fits_path: str, maxN=None):
    ...
    missing = sorted(set(COL.values()) - set(tab.colnames))
    if missing:
        raise ValueError(f"Missing FITS columns: {missing}")
    if len(tab) == 0:
        raise ValueError("The lens table is empty.")
    for name in COL.values():
        values = np.asarray(np.ma.filled(tab[name], np.nan), dtype=float)
        if not np.all(np.isfinite(values)):
            raise ValueError(f"Column {name} contains masked or non-finite values.")
    if np.any(tab["zl"] <= 0) or np.any(tab["zs"] <= tab["zl"]):
        raise ValueError("Require 0 < zl < zs for every lens.")
    for name in set(COL.values()) - {"zl", "zs"}:
        if np.any(tab[name] <= 0):
            raise ValueError(f"Column {name} must be strictly positive.")
...
def fullhier_model(
    ...
    *,
    ...
    log_sig_g = numpyro.sample("log_sig_g", dist.Uniform(-6.0, log_sig_g_max))
    log_sig_d = numpyro.sample("log_sig_d", dist.Uniform(-6.0, log_sig_d_max))
    ...
    # ANN ratio is treated as lens-population prior: constrain lens-side ln(dr_obs), not cosmology-side ln(dr_th).
    ann_resid = (ln_dr_obs - _safe_log(dr_ann)) / jnp.maximum(ann_std, 1e-30)
...
def save_checkpoint(path, *, rng_key, z, step_size, inv_mass, acc, acc_div, recent_queue):
    payload = {
        "acc": acc,
        "acc_div": acc_div,
        "recent_queue": list(recent_queue),
...
def load_checkpoint(path):
    ...
    if "acc" not in payload:
        raise ValueError("Legacy checkpoint lacks posterior history; start a new output directory.")
    return rng_key, z, step_size, inv_mass, payload
...
def main():
    ...
    args = ap.parse_args()
    for name in ("warmup", "chunk", "max_chunks", "max_tree_depth", "host_devices", "recent_chunks", "bad_chunk_warmup"):
        if getattr(args, name) <= 0:
            ap.error(f"--{name} must be positive")
    if args.dz <= 0 or args.maxN < 0 or args.zmax_pad < 0:
        ap.error("Require dz > 0, maxN >= 0 and zmax_pad >= 0")
    if not 0 < args.target_accept < 1:
        ap.error("--target_accept must lie between 0 and 1")
    ...
    # Refuse accidental replacement or continuation with different data/settings.
    manifest_path = Path(ckpt_dir) / "run_config.json"
    config = vars(args).copy()
    for key in ("resume", "max_chunks", "min_ESS", "outdir", "fits"):
        config.pop(key)
    config["fits_sha256"] = hashlib.sha256(Path(args.fits).read_bytes()).hexdigest()
    config["script_sha256"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    if manifest_path.exists():
        if not args.resume:
            raise ValueError("Output already exists. Use a new --outdir or explicit --resume.")
        if json.loads(manifest_path.read_text()) != config:
            raise ValueError("Checkpoint data/configuration differs from this run.")
    elif args.resume:
        raise ValueError("No matching run manifest; start a fresh run without --resume.")
    ...
    if args.resume and os.path.exists(ckpt_path):
        rng_key, z0, step_size, inv_mass, history = load_checkpoint(ckpt_path)
    ...
    if history is not None:
        acc = {k: np.asarray(history["acc"][k]) for k in keep_keys}
        acc_div = np.asarray(history["acc_div"], dtype=bool)
        recent_queue.extend(history["recent_queue"])
    ...
    for ci in range(int(args.max_chunks)):
        ...
        if int(args.ckpt_every) > 0 and ((ci + 1) % int(args.ckpt_every) == 0):
            save_checkpoint(
                ...
                step_size=step_size,
                inv_mass=inv_mass,
                acc=acc, acc_div=acc_div, recent_queue=recent_queue,
            )
    ...
    if not acc[keep_keys[0]].size:
        raise RuntimeError("No accepted samples; inspect diagnostics and adjust sampling settings.")
