...
- Added backend selector `--jax_platform auto|cpu|gpu` (pre-parsed before importing jax).
...
# Early CLI pre-parse: choose JAX platform before importing jax.
_pre_ap = argparse.ArgumentParser(add_help=False)
_pre_ap.add_argument("--jax_platform", choices=["auto", "cpu", "gpu"], default="auto")
_pre_args, _ = _pre_ap.parse_known_args(sys.argv[1:])
if _pre_args.jax_platform == "cpu":
    os.environ.setdefault("JAX_PLATFORMS", "cpu")
elif _pre_args.jax_platform == "gpu":
    # Prefer CUDA GPU; fallback to CPU if CUDA backend is unavailable.
...
def main():
    ...
    ap.add_argument("--jax_platform", default="auto", choices=["auto", "cpu", "gpu"],
                    help="JAX backend preference. Applied at startup before jax import.")
    ...
    print(f"[INFO] requested_jax_platform={args.jax_platform}")
    print(f"[INFO] JAX_PLATFORMS={os.environ.get('JAX_PLATFORMS', '(default)')}")
