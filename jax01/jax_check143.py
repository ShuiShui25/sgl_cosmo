...
def fullhier_model(
    ...
    *,
    ...
    use_normal_intercept_prior=False,
    intercept_prior_sigma=0.15,
    log_sig_g_max=-0.1,
    log_sig_d_max=-0.1,
):
    N = zl.shape[0]
    ...
    if cosmo_model == "wcdm":
        # Sigmoid-mapped constrained sampling: w in (-4, 2)
        w = numpyro.sample(
            "w",
            dist.TransformedDistribution(
                dist.Normal(0.0, 1.0),
                transforms.ComposeTransform(
                    [transforms.SigmoidTransform(), transforms.AffineTransform(-4.0, 6.0)]
                ),
            ),
        )
        cosmo_params = (Om, w, Or)
    elif cosmo_model == "w0wa":
        # Sigmoid-mapped constrained sampling: w0 in (-4, 2), wa in (-6, 6)
        w0 = numpyro.sample(
            "w0",
            dist.TransformedDistribution(
                dist.Normal(0.0, 1.0),
        ...
        wa = numpyro.sample(
            ...
            dist.TransformedDistribution(
                dist.Normal(0.0, 1.0),
                transforms.ComposeTransform(
                    [transforms.SigmoidTransform(), transforms.AffineTransform(-6.0, 12.0)]
                ),
            ),
        )
    ...
    gamma_lat = gamma0 + gamma_s * zlc + sig_g * u_g
    delta_lat = delta0 + delta_s * zlc + sig_d * u_d
    gamma_i = 1.71 + (2.50 - 1.71) * jax.nn.sigmoid(0.3 * gamma_lat)
    delta_i = 1.81 + (2.80 - 1.81) * jax.nn.sigmoid(0.3 * delta_lat)
...
def main():
    ...
    ap.add_argument("--ann_weight", type=float, default=1.0, help="Weight multiplier for ANN log-likelihood term.")
    ...
    ap.add_argument("--use_normal_intercept_prior", action="store_true",
                    help="Use truncated normal priors for gamma0 and delta0.")
    ap.add_argument("--intercept_prior_sigma", type=float, default=0.15,
    ...
    print(f"[INFO] ann_weight={float(args.ann_weight):.3f}, zl_scale_mode={args.zl_scale_mode}")
