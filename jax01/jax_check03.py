...
def E_z_flat_w0wa(z, Om, w0, wa):
    """
    E(z)=H(z)/H0 for flat CPL w0waCDM:
      w(z)=w0 + wa*z/(1+z)
...
def build_I_of_z_grid(cosmo_model, cosmo_params, z_grid):
    ...
    if cosmo_model == "wcdm":
        ...
        Om, w0, wa = cosmo_params
        Ez = E_z_flat_w0wa(z_grid, Om, w0, wa)
    else:
...
def fullhier_model(
    ...
    *,
    ...
    if cosmo_model == "wcdm":
        ...
        # You can tighten these if you want; kept broad like your w prior.
        w0 = numpyro.sample("w0", dist.Uniform(-4.0, 2.0))
        wa = numpyro.sample("wa", dist.Uniform(-6.0, 6.0))
        cosmo_params = (Om, w0, wa)
    else:
        raise ValueError(f"Unknown cosmo_model={cosmo_model}. Use 'wcdm' or 'w0wa'.")
...
def main():
    ...
    # cosmology switch
    ap.add_argument("--cosmo_model", default="wcdm", choices=["wcdm", "w0wa"],
                    help="Cosmology model: wcdm (Om,w) or w0wa (Om,w0,wa).")
