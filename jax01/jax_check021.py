...
def ln_triangular_pdf_soft(x, left, mode, right, tiny=1e-30):
    """
    Triangular pdf in log-space, but returns a finite very negative value outside support
    by flooring pdf with 'tiny'. This avoids hard -inf steps (still gives strong penalty).
    """
    x = jnp.asarray(x)
    num1 = 2.0 * (x - left)
    ...
    # outside support -> pdf <= 0; floor to tiny
    pdf = jnp.maximum(pdf, tiny)
    return jnp.log(pdf)
...
def fullhier_model(
    ...
    *,
    ...
    d_thetaE=0.05,
    min_rel_sn=1e-4,
    bound_eps=0.02,  # <--- NEW: smooth boundary sharpness
):
    N = zl.shape[0]
    eps = jnp.asarray(bound_eps, dtype=zl.dtype)

    # ---- Priors (unchanged; numpyro will do constrained transforms internally) ----
    ...
    log_sig_g = numpyro.sample("log_sig_g", dist.Uniform(-10.0, -0.1))

    delta0 = numpyro.sample("delta0", dist.Uniform(1.7, 3.0))
    delta_s = numpyro.sample("delta_s", dist.Uniform(-3.0, 3.0))
    log_sig_d = numpyro.sample("log_sig_d", dist.Uniform(-10.0, -0.1))
    ...
    # ---- hierarchical lens-level parameters ----
    gamma_lin = gamma0 + gamma_s * zl + sig_g * u_g    # unconstrained
    gamma_i   = 1.51 + (2.99 - 1.51) * sigmoid(gamma_lin)

    delta_lin = delta0 + delta_s * zl + sig_d * u_d    # unconstrained
    delta_i   = 1.51 + (2.99 - 1.51) * sigmoid(delta_lin)

    beta_lin  = beta0 + sig_b * u_b                     # unconstrained
    beta_i    = -0.5 + (0.656 - (-0.5)) * sigmoid(beta_lin)

    # ============================================================
    # REPLACE "soft_box_logp" (step / cliff) with SMOOTH boundaries
    # ============================================================

    # data sanity constraints (these are fixed by data; keep them as a single check)
    data_ok = (
        jnp.isfinite(zl) & jnp.isfinite(zs) & (zs > zl) &
        jnp.isfinite(thetaE) & (thetaE > 0.0) &
        jnp.isfinite(theta_ap) & (theta_ap > 0.0) &
        jnp.isfinite(sigma_ap) & (sigma_ap > 0.0) &
        jnp.isfinite(sigma_ap_err) & (sigma_ap_err > 0.0)
    )
    # If data itself is invalid, kill logp (this should never trigger for a clean table)
    numpyro.factor("data_ok", jnp.sum(jnp.where(data_ok, 0.0, -1e20)))

    # ---- smooth physical/model-valid boxes ----
    # gamma_i in (1.01, 2.99)
    lp_g = log_sigmoid((gamma_i - 1.01) / eps) + log_sigmoid((2.99 - gamma_i) / eps)

    # delta_i in (0.51, 2.99)  (also avoids d ~ 3 singularity)
    lp_d = log_sigmoid((delta_i - 0.51) / eps) + log_sigmoid((2.99 - delta_i) / eps)

    # beta: use triangle support as soft box (preferred)
    left, mode, right = -0.5, 0.102, 0.656
    lp_b = log_sigmoid((beta_i - left) / eps) + log_sigmoid((right - beta_i) / eps)

    # Combine soft bound penalties
    # These are smooth (no step), and become sharper as bound_eps -> 0.
    numpyro.factor("soft_bounds", jnp.sum(lp_g + lp_d + lp_b))
    ...
    # ---- beta triangular prior (soft outside) ----
    beta_lp = ln_triangular_pdf_soft(beta_i, left, mode, right)
...
def compute_ess_dict(acc_samples: dict, keys):
    ...
    for k in keys:
        ...
        if x is None or len(x) < 20:
            ...
            x2 = jnp.asarray(x)[None, :]  # (chains=1, draws)
...
def main():
    ...
    # smooth boundary sharpness
    ap.add_argument("--bound_eps", type=float, default=0.02,
                    help="Smooth-boundary sharpness (smaller -> closer to hard wall).")
    ...
    def model():
        return fullhier_model(
            zl, zs, thetaE, theta_ap, sigma_ap, sigma_ap_err, dr_ann, dr_ann_err,
            z_grid=z_grid,
            bound_eps=float(args.bound_eps),
        )
    ...
    out_path = os.path.join(args.outdir, "posterior_minimal_chunks.npz")
