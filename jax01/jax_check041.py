...
- Explicit validity check in `f_prime_jax`: returns `(fp, fp_ok)` and penalizes invalid samples.
- `wphi` uses a continuous expression in alpha via `expm1/log` (no piecewise alpha-branch).
- Added multi-chain CLI controls: `--num_chains`, `--chain_method`.
- Applied medium-priority performance updates:
  - JIT wrappers for `I(z)` and `dr_th` path.
  - ESS computed every `--ess_every` chunks.
  - Sample accumulation uses list-of-arrays (avoids repeated O(n^2) concatenation).
...
def E_z_flat_wphi(z, Om, w0, alpha, Or=0.0):
    ...
    Uses a continuous alpha expression with expm1/log for better numerical behavior.
    """
    zp1 = 1.0 + z
    Or = jnp.asarray(Or)
    Ode = 1.0 - Om - Or
    ...
    log_zp1 = _safe_log(zp1)
    q = -jnp.expm1(-alpha * log_zp1) / jnp.maximum(alpha, 1e-12)
    f_de = jnp.exp(3.0 * (1.0 + w0) * q)
...
# JIT wrappers: cosmo_model is static for build_I_of_z_grid.
build_I_of_z_grid_jit = jax.jit(build_I_of_z_grid, static_argnames=("cosmo_model",))
dr_th_from_I_jit = jax.jit(dr_th_from_I)
...
def f_prime_jax(g, d, beta):
    ...
    Returns:
      fp: computed f' with guarded denominator
      fp_ok: boolean mask requiring denominator > 0 and finite quantities
    """
    g = jnp.asarray(g)
    d = jnp.asarray(d)
    ...
    den_ok = jnp.isfinite(den) & (den > 0.0)
    den_safe = jnp.where(den_ok, den, 1.0)
    fp = pref * term1 * jnp.exp(ln_num) * term3 / den_safe
    fp_ok = den_ok & jnp.isfinite(fp) & (fp > 0.0)
    return fp, fp_ok


def ln_dd_obs_jax(g, d, beta, thetaE, theta_ap, sigma_ap):
    fp, fp_ok = f_prime_jax(g, d, beta)
    ...
    dd_ok = fp_ok & jnp.isfinite(dd) & (dd > 0.0)
    ln_dd = _safe_log(jnp.where(dd_ok, dd, 1.0))
    return ln_dd, dd_ok
...
def fullhier_model(
    ...
    *,
    ...
    if cosmo_model == "wcdm":
        ...
        # Broad priors (adjust if you want)
    ...
    I_grid = build_I_of_z_grid_jit(cosmo_model, cosmo_params, z_grid)
    dr_th = dr_th_from_I_jit(zl, zs, z_grid, I_grid)
    ln_dr_th = _safe_log(dr_th)

    # ---- ln(dr_obs) from lensing+dynamics ----
    ln_dr_obs, dd_ok = ln_dd_obs_jax(gamma_i, delta_i, beta_i, thetaE, theta_ap, sigma_ap)
    numpyro.factor("fprime_valid", jnp.sum(jnp.where(dd_ok, 0.0, -1e20)))
...
def _format_step_size(step_size):
    arr = _to_host(step_size)
    arr = np.asarray(arr)
    if arr.ndim == 0:
        return f"{float(arr):.3e}"
    return f"shape={arr.shape}, min={arr.min():.3e}, max={arr.max():.3e}, mean={arr.mean():.3e}"


def _reduce_chain_param(x, num_chains):
    arr = _to_host(x)
    arr = np.asarray(arr)
    if arr.ndim >= 1 and num_chains > 1 and arr.shape[0] == num_chains:
        return jnp.asarray(arr.mean(axis=0))
    return x


def _reduce_chain_pytree(tree, num_chains):
    return jax.tree_util.tree_map(lambda a: _reduce_chain_param(a, num_chains), tree)
...
# diagnostics
# -----------------------------
def compute_ess_dict(acc_samples: dict, keys, min_draws=20):
    """
    Compute ESS from accumulated chunk lists.

    acc_samples[k] is a list of arrays with shape (num_chains, chunk_draws).
    Returns:
      ess_per_key (dict),
      min_ess (float),
    ...
    for k in keys:
        chunks = acc_samples.get(k, None)
        if not chunks:
            ess = np.nan
        else:
            x = np.concatenate(chunks, axis=1)  # (num_chains, total_draws)
            if x.shape[1] < int(min_draws):
                ess = np.nan
            else:
                ess_val = effective_sample_size(jnp.asarray(x, dtype=jnp.float32))
...
def get_chunk_extra_diagnostics(mcmc):
    """
    ...
    try:
        ...
        return accept_mean, tree_depth_mean, ebfmi
...
def main():
    ...
    ap.add_argument("--num_chains", type=int, default=1, help="Number of MCMC chains.")
    ap.add_argument("--chain_method", default="sequential", choices=["sequential", "parallel", "vectorized"],
                    help="Chain execution mode for NumPyro MCMC.")
    ...
    ap.add_argument("--ess_every", type=int, default=1,
                    help="Compute accumulated ESS every N chunks (>=1).")
    ...
    def _make_mcmc(kernel, num_warmup, num_samples):
        sig = inspect.signature(MCMC.__init__)
        kwargs = dict(
        ...
        fields = ("accept_prob", "tree_depth", "energy", "num_steps", "diverging")
        if "extra_fields" in sig.parameters:
            kwargs["extra_fields"] = fields
        elif "collect_fields" in sig.parameters:
            kwargs["collect_fields"] = fields
    ...
    print(f"[INFO] num_chains={args.num_chains}, chain_method={args.chain_method}")
    ...
    if args.resume and os.path.exists(ckpt_path):
        ...
        step_size = _reduce_chain_param(step_size, int(args.num_chains))
        inv_mass = _reduce_chain_pytree(inv_mass, int(args.num_chains))
        print(f"[INFO] Resumed from checkpoint: {ckpt_path}")
        print(f"[INFO] step_size={_format_step_size(step_size)}")
        do_warmup = False
    else:
        do_warmup = True
    ...
    # accumulate per-key chunks as (num_chains, chunk_draws) arrays.
    acc = {k: [] for k in keep_keys}
    acc_div = []

    ess_every = max(1, int(args.ess_every))
    min_ess = np.nan
    stopped_by = None
    ...
    for ci in range(int(args.max_chunks)):
        ...
        if do_warmup:
            ...
            step_size_raw, inv_mass_raw = _extract_adapt_params(state)
            step_size = _reduce_chain_param(step_size_raw, int(args.num_chains))
            inv_mass = _reduce_chain_pytree(inv_mass_raw, int(args.num_chains))
            z0 = state.z
            do_warmup = False

            print(f"[INFO] Learned step_size={_format_step_size(step_size_raw)} from warmup; will reuse reduced inv_mass thereafter.")
        ...
        accept_mean, tree_depth_mean, ebfmi = get_chunk_extra_diagnostics(mcmc)
        if accept_mean is None:
            print("[DIAG] acceptance mean: (not available)")
        else:
            print(f"[DIAG] acceptance mean (this chunk) = {accept_mean:.4f}")
        if tree_depth_mean is None:
            print("[DIAG] tree depth mean: (not available)")
        else:
            print(f"[DIAG] tree depth mean (this chunk) = {tree_depth_mean:.2f}")
        if ebfmi is None:
        ...
        for k in keep_keys:
            if k in s:
                new = np.array(_to_host(s[k]), dtype=np.float64)
                if new.ndim == 1:
                    new = new[None, :]
                elif new.ndim > 2:
                    # keep scalar monitored params only; flatten any trailing dims.
                    new = new.reshape(new.shape[0], new.shape[1], -1)[..., 0]
                acc[k].append(new)

        # ---- diagnostics: ESS (accumulated) ----
        do_ess = ((ci + 1) % ess_every == 0) or ((ci + 1) == int(args.max_chunks))
        if do_ess:
            ess_per, min_ess, min3 = compute_ess_dict(acc, keep_keys)
            print(f"[DIAG] ESS (accumulated): min ESS over {len(keep_keys)} params = {min_ess:.1f}")
            if min3:
                msg = ", ".join([f"({idx},{name},{ess:.1f})" for idx, name, ess in min3])
                print(f"[DIAG] ESS min-3 (index,param,ESS) = {msg}")
            else:
                print("[DIAG] ESS min-3: (not available)")
        else:
            print(f"[DIAG] ESS skipped on chunk {ci + 1}; compute every {ess_every} chunks.")
        ...
        if do_ess and float(args.min_ESS) > 0.0 and np.isfinite(min_ess) and (min_ess >= float(args.min_ESS)):
    ...
    for k in keep_keys:
        if acc[k]:
            out[k] = np.concatenate(acc[k], axis=1).reshape(-1)
        else:
            out[k] = np.empty((0,), dtype=np.float64)
    if acc_div:
        out["diverging"] = np.concatenate(acc_div, axis=1).reshape(-1)
    else:
        out["diverging"] = np.empty((0,), dtype=bool)
